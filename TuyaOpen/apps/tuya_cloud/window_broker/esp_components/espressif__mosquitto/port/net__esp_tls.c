...
struct mosquitto *net__socket_accept(struct mosquitto__listener_sock *listensock)
{
    ...
    /* 启用 TCP Keepalive：检测死连接，防止客户端在网路静默断开后 broker 仍保持半开连接 */
    {
        int keepalive = 1;
        setsockopt(new_sock, SOL_SOCKET, SO_KEEPALIVE, &keepalive, sizeof(keepalive));
        int idle = 30;   /* 首次探测前的空闲时间（秒） */
        setsockopt(new_sock, IPPROTO_TCP, TCP_KEEPIDLE, &idle, sizeof(idle));
        int interval = 10;  /* 探测间隔（秒） */
        setsockopt(new_sock, IPPROTO_TCP, TCP_KEEPINTVL, &interval, sizeof(interval));
        int count = 3;   /* 探测失败次数后断开 */
        setsockopt(new_sock, IPPROTO_TCP, TCP_KEEPCNT, &count, sizeof(count));
    }
