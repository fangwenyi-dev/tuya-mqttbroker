...
int __wrap_mosquitto_unpwd_check(struct mosquitto *context)
{
    ...
    if (g_mosq_connect_callback) {
        ...
        /* User callback accepted the connection (returned 0).
         * Skip __real_mosquitto_unpwd_check to avoid double authentication conflict.
         * The real function may reject the connection because no password_file is configured,
         * overriding the user callback's decision. This is the root cause of MQTTX frequent
         * disconnections: the user callback approves, but the real function defers/rejects,
         * causing the client to be in an inconsistent state and eventually disconnected. */
        return MOSQ_ERR_SUCCESS;
    }

    /* No user callback registered, fall through to original function */
